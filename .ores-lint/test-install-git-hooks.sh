...
(cd "$repo" && git config core.hooksPath .githooks && mkdir -p .githooks && printf '%s\n' '#!/bin/sh' 'echo existing' > .githooks/pre-push && chmod +x .githooks/pre-push && before=$(cat .githooks/pre-push) && if sh .ores-lint/install-git-hooks.sh; then echo 'installer overwrote an unrelated pre-push hook' >&2; exit 1; fi && test "$(cat .githooks/pre-push)" = "$before")
