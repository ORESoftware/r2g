...
#   1. Uncomment the "_r2g_log_file=..." line.
#   2. 'tail -f /var/tmp/dashdash-completion.log' in one terminal.
#   3. Re-source this bash completion file.
#_r2g_log=/var/tmp/dashdash-completion.log

function _r2g_completer {

    # ---- cmd definition

    local cmd_shortopts="-h -h -h -s -t -v -v -v -z"
local cmd_longopts="--allow-unknown --allow-unknown --allow-unknown --bash-completion --completion --docker --full --help --help --help --keep --multi --pack --search --search --search-root --search-root --skip --verbosity --verbosity --verbosity --version --vn"
local cmd_optargs="--search-root=string --search-root=string --search=string --search=string --skip=string --verbosity=integer --verbosity=integer --verbosity=integer -v=integer -v=integer -v=integer"
...
if type complete &>/dev/null; then
    function _r2g_completion {
        local _log_file=/dev/null
        [[ -z "$_r2g_log" ]] || _log_file="$_r2g_log"
        COMPREPLY=($(COMP_CWORD="$COMP_CWORD" \
            COMP_LINE="$COMP_LINE" \
            COMP_POINT="$COMP_POINT" \
            _r2g_completer -- "${COMP_WORDS[@]}" \
            2>$_log_file)) || return $?
    }
    complete -o default -F _r2g_completion r2g
    ...
    function _r2g_completion {
        ...
        compadd -- $(COMP_CWORD=$((CURRENT-1)) \
            COMP_LINE=$BUFFER \
            COMP_POINT=0 \
            _r2g_completer -- "${words[@]}" \
            2>$_log_file)
    }
    compdef _r2g_completion r2g
    ...
    compctl -K _r2g_completion r2g
fi


##
## This is a Bash completion file for the 'r2g' command. You can install
## with either:
##
##     cp FILE /usr/local/etc/bash_completion.d/r2g   # Mac
##     cp FILE /etc/bash_completion.d/r2g             # Linux
##
## or:
##
##     cp FILE > ~/.r2g.completion
##     echo "source ~/.r2g.completion" >> ~/.bashrc
##
